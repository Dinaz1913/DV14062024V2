...
$transactionBuyFilePath = 'transactions_buy.json';
$transactionSellFilePath = 'transactions_sell.json';
...
$transactionHistoryBuy = new TransactionHistory($transactionBuyFilePath);
$transactionHistorySell = new TransactionHistory($transactionSellFilePath);
        ...
        case 3:
            ...
            if ($crypto === null) {
                ...
                if (strtolower($choice) === 'yes') {
                    ...
                    $transactionHistoryBuy->addTransaction($username, date('Y-m-d H:i:s'), 'buy', $crypto->getSymbol(), $amount, $crypto->getQuote()->getPrice(), $crypto->getQuote()->getPrice() * $amount);
                    echo "Bought $amount of " . $crypto->getName() . "\n";
                    User::saveUsers($usersFilePath, $users);
                    $transactionHistoryBuy->saveTransactions();
                }
            }
            break;
        ...
        case 4:
            ...
            if ($crypto === null) {
                ...
                try {
                    ...
                    $transactionHistorySell->addTransaction($username, date('Y-m-d H:i:s'), 'sell', $crypto->getSymbol(), $amount, $crypto->getQuote()->getPrice(), $crypto->getQuote()->getPrice() * $amount);
                    echo "Sold $amount of " . $crypto->getSymbol() . "\n";
                    User::saveUsers($usersFilePath, $users);
                    $transactionHistorySell->saveTransactions();
                } catch (\Exception $e) {
                    echo $e->getMessage() . "\n";
                }
            }
